...
....
git clone https://github.com/devonfw/oasp4js-ng2-project-seed.git
....
...
Uninstall Bootstrap npm dependency:
----
npm uninstall bootstrap-css-only --save
----

Remove copying bootstrap fonts while building from `gulpfile.js`: remove the `copy-bootstrap-fonts` task itself as well
as any dependencies to it (from tasks `build` and `build:dist`).

Remove importing Bootstrap from `styles.less`.

Install Angular Material npm dependency:
----
...
Import Angular Material in the `AppModule` module (`app.module.ts`):
[source,javascript]
----
...
